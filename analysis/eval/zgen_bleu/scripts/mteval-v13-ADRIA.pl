...
sub check_MT_data
{
	...
	warn "Not the same 'setid' attribute values across files" unless ( ( $src_id eq $tst_id ) && ( $src_id eq $ref_id ) );
...
#score each segment
	foreach my $seg ( sort{ $a <=> $b } keys( %{$tst_data{$sys}{$doc}{SEGS}} ) )
	{
		...
		if ( $method eq "BLEU" )
		{
			my %DOCmt = ();
			my $segScore = 0;
			if ($tst_cnt->[1] != 0){ $segScore = &{$BLEU_SCORE}($ref_length, $match_cnt, $tst_cnt, $sys, %DOCmt); }
...
sub bleu_score_nosmoothing
{
	...
	for ( my $j = 1; $j <= $max_Ngram; ++$j )
	{
		...
		else
		{
			...
			$SCOREmt->{ $j }{ $sys }{ cum_wo_length_penalty } = exp( $score / $j ); # ADRIA
			$SCOREmt->{ $j }{ $sys }{ cum_length_penalty } = exp( $len_score ); # ADRIA
...
sub printout_report
{
	...
	elsif ($METHOD eq "BLEU" )
	{
		foreach my $sys (sort @tst_sys)
		{
		    #printf "\nBLEU score = %.4f for system \"$sys\"\n",$BLEUmt{4}{$sys}{cum}; # ADRIA
		    printf "\nBLEU score = %.4f (%.4f * %.4f) for system \"$sys\"\n",$BLEUmt{4}{$sys}{cum}, $BLEUmt{4}{$sys}{cum_wo_length_penalty}, $BLEUmt{4}{$sys}{cum_length_penalty};
		}
	}
	printf "\n# ------------------------------------------------------------------------\n\n";
